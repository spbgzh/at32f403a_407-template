...
void spim_init(void)
{
	...
	/* enable the clock */
	crm_periph_clock_enable(CRM_IOMUX_PERIPH_CLOCK, TRUE);		//����MUX����ʱ��
	crm_periph_clock_enable(CRM_GPIOA_PERIPH_CLOCK, TRUE);		//����gpioʱ��
	crm_periph_clock_enable(CRM_GPIOB_PERIPH_CLOCK, TRUE);		//����gpiobʱ��

	/* init spim io */
	gpio_default_para_init(&gpio_init_struct);
	gpio_init_struct.gpio_drive_strength = GPIO_DRIVE_STRENGTH_STRONGER;
	gpio_init_struct.gpio_out_type  = GPIO_OUTPUT_PUSH_PULL;				//�������ģʽ
...
void spim_operate(void)
{
	...
	{
		/* test spim fail */
		printf ("operate test spim fail \r\n");
		return;
	}
	/* erase an spim flash sector */
	if(flash_sector_erase(SPIM_TEST_ADDR) != FLASH_OPERATE_DONE)		//������ÿҳ4KB
	{
		/* test spim fail */
		printf ("operate test spim fail \r\n");
		return;
	...
	for(i = 0; i < SPIM_SECTOR_SIZE; i++)							//����Ƿ�����ɹ�
	{
	...
	i = 0;
	while(i < SPIM_SECTOR_SIZE)									//��SPIM_TEST_ADDR��ַ��ʼд��write_buffer�е����ݣ�����ΪSPIM_SECTOR_SIZE��4KB��
	{
		status = flash_word_program(SPIM_TEST_ADDR + i, *(uint32_t *)(write_buffer + i));			//������д��
	...
	/* read an spim flash sector */
	memset(read_buffer, 0, SPIM_SECTOR_SIZE);
	sector_read(SPIM_TEST_ADDR, SPIM_SECTOR_SIZE, read_buffer);			//��SPIM_TEST_ADDR��ַ��ʼ�������ݵ�read_buffer�У�����ΪSPIM_SECTOR_SIZE��4KB��	
	/* check if reading result and writing content are the same */
	for(i = 0; i < SPIM_SECTOR_SIZE; i++)								//�Ա�д������ݺͶ���������
	{
	...
	printf ("operate test spim scuss \r\n");
